...
const TAKSA_RULES = {
    ...
    0: { 
        ...
        vowels: "ะาิีึืุูเแอโอไอใ", // บริวารวันอาทิตย์
        kala: "ศษสหฬฮ",
        kalaType: "อักขระ"
    },
    1: { 
        ...
        vowels: "ะาิีึืุูเแอโอไอใ", // กาลกิณีวันจันทร์
        kala: "อะอาอิอีอุอูเแอโอไอใ", // สำหรับสระ
...
function getDayOfWeek(date) {
    ...
    // สำหรับวันพฤหัสบดี 00:00 - 05:59 ก็ยังนับเป็นวันพุธกลางคืนตามหลักทักษา แต่เพื่อความง่ายจะยึดตามวันจริง
    // ในระบบนี้จะใช้ getDay() ตรงๆ ยกเว้นวันพุธ
        ...
        if (!isKala) {
            ...
            if (category === 'ไม่จัดประเภท' && taksaData.vowels && taksaData.vowels.includes(char)) {
                 if (birthDayIndex === 0) { // สระคือ บริวาร วันอาทิตย์
                    ...
                    if (char !== 'เ' && char !== 'แ' && char !== 'โ' && char !== 'ไ' && char !== 'ใ') { // สระบางตัวอาจถูกจัดเป็นอักขระ
                        isKala = true;
                        kalaCharacters.push(char);
                        category = 'กาลกิณี';
                    } else {
                         // สำหรับวันจันทร์ สระส่วนใหญ่คือ กาลกิณี
                         isKala = true;
                         kalaCharacters.push(char);
                         category = 'กาลกิณี';
                    }
...
// ==============================================
// 🔮 NEW: ฟังก์ชันทำนายชื่อ-นามสกุล (หลักเลขศาสตร์)
// ==============================================
...
window.calculateNumerology = function() {
    ...
    const categories = Object.keys(taksaResult.breakdown).filter(cat => taksaResult.breakdown[cat].length > 0);
    
    // เรียงตามลำดับทักษาที่ต้องการ
...
// (*** นำโค้ดส่วนอื่น ๆ ที่ไม่เกี่ยวข้องกับ Numerology/Taksā มาวางต่อท้ายใน script.js ***)
// (ฟังก์ชัน calculatePersonalInfo, WorldClock, Timer/Pause/Reset)
// (*** เพื่อให้โค้ด script.js สมบูรณ์เหมือนเดิม ***)
